...
fn parse_args(args: Vec<String>) -> Result<RmFlags> {
    ...
    for arg in parse_arg_kinds(&args) {
        match arg {
            ...
            ArgKind::LongFlag(_) | ArgKind::ShortFlag(_) => {
...
mod test {
    ...
    async fn test_force() {
        ...
        let result =
            execute_remove(dir.path(), vec!["non_existent.txt".to_string()])
                .await;
        assert_eq!(
    ...
    async fn test_recursive() {
        ...
        execute_remove(
            dir.path(),
            vec!["-r".to_string(), "folder".to_string()],
        )
        .await
        .unwrap();
        assert!(!sub_file.exists());
        ...
        execute_remove(
            dir.path(),
            vec!["-rf".to_string(), "folder".to_string()],
        )
        .await
        .unwrap();
    }
