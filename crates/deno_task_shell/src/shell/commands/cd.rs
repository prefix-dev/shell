    ...
    fn execute(
        ...
        Box::pin(async move {
            match execute_cd(
                ...
                Ok(new_dir) => ExecuteResult::Continue(
                    0,
                    vec![EnvChange::Cd(new_dir)],
                    Vec::new(),
                ),
                Err(err) => {
...
mod test {
    ...
    fn test_cd_previous() {
        ...
        let result =
            execute_cd(&dir_path, sub_dir.as_ref(), vec!["-".to_string()])
                .unwrap();
    ...
    fn test_directory_navigation() {
        ...
        // Test cd -
        let result =
            execute_cd(&dir_path, prev_dir.as_ref(), vec!["-".to_string()])
                .unwrap();
