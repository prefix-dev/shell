...
fn execute_continue(args: Vec<String>) -> Result<i32> {
    ...
    // For now, we only support continuing in the innermost loop
    // TODO: Support continuing in n levels of loops
    Ok(0)
}
...
mod test {
    ...
    fn executes_continue() {
        assert_eq!(execute_continue(vec![]).unwrap(), 0);
        assert_eq!(execute_continue(vec!["1".to_string()]).unwrap(), 0);
    }
}
