...
async fn resolve_command<'a>(
    ...
    if command_name.name.contains('/') {
        if let Some(shebang) =
            resolve_shebang(&command_path).map_err(|err| {
                ResolveCommandError::FailedShebang(
                    FailedShebangError::MietteError(err.to_string()),
                )
            })?
        {
            let (shebang_command_name, mut args) = if shebang.string_split {
                let mut args = parse_shebang_args(&shebang.command, context)
                    .await
                    .map_err(|e| {
                        FailedShebangError::MietteError(e.to_string())
                    })?;
...
async fn parse_shebang_args(
    ...
    let cmd = match cmd.inner {
        ...
        CommandInner::ConditionExpression(_) => return err_unsupported(text),
    };
...
mod local_test {
    ...
    fn should_error_on_unknown_command() {
        let cwd = std::env::current_dir().unwrap();
        let state =
            ShellState::new(Default::default(), &cwd, Default::default());
        // Command not found
