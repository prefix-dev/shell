...
fn parse_list(pair: Pair<Rule>, items: &mut Vec<SequentialListItem>) -> Result<()> {
...
fn parse_compound_list(pair: Pair<Rule>, items: &mut Vec<SequentialListItem>) -> Result<()> {
    for item in pair.into_inner() {
        match item.as_rule() {
            Rule::term => {
...
fn parse_term(pair: Pair<Rule>, items: &mut Vec<SequentialListItem>) -> Result<()> {
    for item in pair.into_inner() {
        match item.as_rule() {
            Rule::and_or => {
                let sequence = parse_and_or(item)?;
    ...
    match items.next() {
        Some(next_item) => {
            if next_item.as_rule() == Rule::ASSIGNMENT_WORD {
                anyhow::bail!("Multiple assignment words before && or || is not supported yet");
            } else {
            ...
            Rule::cmd_suffix => {
                for suffix in item.into_inner() {
                    match suffix.as_rule() {
                        ...
                        Rule::QUOTED_WORD => args.push(Word::new(vec![parse_quoted_word(suffix)?])),
                        _ => {
                            return Err(anyhow::anyhow!(
...
fn parse_subshell(pair: Pair<Rule>) -> Result<Command> {
    ...
    if let Some(inner) = pair.into_inner().next() {
        ...
        Err(anyhow::anyhow!(
            "Unexpected end of input in subshell"
        ))
    }
}
        ...
        Rule::UNQUOTED_PENDING_WORD => {
            for part in pair.into_inner() {
                match part.as_rule() {
                    ...
                    Rule::SUB_COMMAND => {
                        let command = parse_complete_command(part.into_inner().next().unwrap())?;
                        parts.push(WordPart::Command(command));
                    },
                    Rule::VARIABLE => parts.push(WordPart::Variable(part.as_str().to_string())),
                    Rule::QUOTED_WORD => {
                        let quoted = parse_quoted_word(part)?;
                        parts.push(quoted);
                    },
                    _ => return Err(anyhow::anyhow!("Unexpected rule in UNQUOTED_PENDING_WORD: {:?}", part.as_rule())),
                }
            }
        },
        Rule::QUOTED_WORD => {
...
fn parse_quoted_word(pair: Pair<Rule>) -> Result<WordPart> {
    ...
    match inner.as_rule() {
        Rule::DOUBLE_QUOTED => {
            ...
            for part in inner.into_inner() {
                match part.as_rule() {
                    ...
                    Rule::VARIABLE => parts.push(WordPart::Variable(part.as_str()[1..].to_string())),
                    Rule::QUOTED_CHAR => {
                        if let Some(WordPart::Text(ref mut s)) = parts.last_mut() {
                            s.push_str(part.as_str());
                        } else {
                    ...
                    _ => return Err(anyhow::anyhow!("Unexpected rule in DOUBLE_QUOTED: {:?}", part.as_rule())),
                }
            };
        ...
        Rule::SINGLE_QUOTED => {
            ...
            Ok(WordPart::Quoted(vec![WordPart::Text(trimmed_str.to_string())]))
        },
        _ => Err(anyhow::anyhow!("Unexpected rule in QUOTED_WORD: {:?}", inner.as_rule())),
    }
}
